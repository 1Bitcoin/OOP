...
Lift_doors::Lift_doors(QObject *parent) : QObject(parent), doors_open_timer(std::make_shared<QTimer>()),
    doors_close_timer(std::make_shared<QTimer>()), doors_stay_open_timer(std::make_shared<QTimer>())
{
    ...
    QObject::connect(doors_open_timer.get(), SIGNAL(timeout()), this, SLOT(open()));
    QObject::connect(doors_close_timer.get(), SIGNAL(timeout()), this, SLOT(close()));

    // когда двери открыты, начать режим ожидания
    QObject::connect(this, SIGNAL(opened_doors()), doors_stay_open_timer.get(), SLOT(start()));

    // когда истечет таймер режима ожидания - начать закрываться
    QObject::connect(doors_stay_open_timer.get(), SIGNAL(timeout()), this, SLOT(start_closing()));
}
